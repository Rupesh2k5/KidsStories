...
export const createRazorpayOrder = async (req, res) => {
    try {
        ...
        res.json({ success: true, order: razorpayOrder, advanceAmount, totalPrice });
...
export const createCartRazorpayOrder = async (req, res) => {
    try {
        ...
        res.json({ success: true, order: orderInfo, key_id });
    } catch (err) {
        console.log(err);
        res.json({ success: false, message: err.message });
    }
...
export const verifyCartOrder = async (req, res) => {
    try {
        ...
        for (let item of cart) {
            const newOrder = new order({
                book: item.id.length === 24 ? item.id : '60d5ecb74d6bb830b8e71111',
                user: _id,
                owner: _id,
                pickupDate: new Date(),
                returnDate: new Date(),
